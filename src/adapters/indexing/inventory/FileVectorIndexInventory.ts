...
/**
 * Read the whole section a chunk belongs to (SPEC-corpus-knowledge R2a).
 * Section boundary: the contiguous run of chunks sharing the hit's non-empty
 * headingPath; for sources without headings — a run bounded by title-like
 * chunks (very short text), starting at the nearest title. Output is capped by
 * `maxChars` with a cursor to continue.
 */
export function readFileVectorIndexSection(
  state: FileVectorIndexState,
  options: IndexSectionReadOptions,
): IndexSectionReadResult | null {
  ...
  const headingPath = chunkHeadingPath(hit.row.source);
  const [sectionStart, sectionEnd] =
    headingPath.length > 0
      ? headingSectionBounds(chunks, index, headingPath)
      : heuristicSectionBounds(chunks, index);

  const section = chunks.slice(sectionStart, sectionEnd + 1);
  const cursorStart = Math.min(parseCursor(options.cursor), section.length);
  const selected: IndexChunkReadResult["chunks"] = [];
  let remaining = options.maxChars;
  let consumed = cursorStart;

  for (const chunk of section.slice(cursorStart)) {
    if (remaining <= 0) {
      break;
    }
    const text = chunk.row.text.slice(0, remaining);
  ...
  const lastTruncated = selected.length > 0 && selected[selected.length - 1].truncated;
  const hasMore = consumed < section.length || lastTruncated;
...
/** Contiguous run of chunks sharing the exact heading path. */
function headingSectionBounds(
  chunks: StoredChunk[],
  index: number,
  headingPath: string[],
): [number, number] {
  const key = headingPath.join("\u0000");
  const matches = (chunk: StoredChunk) => chunkHeadingPath(chunk.row.source).join("\u0000") === key;

  let start = index;
  while (start > 0 && matches(chunks[start - 1])) {
    start -= 1;
  }
  let end = index;
  while (end + 1 < chunks.length && matches(chunks[end + 1])) {
    end += 1;
  }
...
function heuristicSectionBounds(chunks: StoredChunk[], index: number): [number, number] {
  const titleLike = (chunk: StoredChunk) => chunk.row.text.trim().length <= TITLE_LIKE_MAX_CHARS;

  let start = index;
  while (start > 0 && !titleLike(chunks[start])) {
    start -= 1;
  }
  let end = index;
  while (end + 1 < chunks.length && !titleLike(chunks[end + 1])) {
    end += 1;
  }
