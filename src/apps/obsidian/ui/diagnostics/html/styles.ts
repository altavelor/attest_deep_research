// NOTE: the same stylesheet serves the standalone exported page and the modal's
// shadow root, so root-level rules are declared for both :root/body and
// :host/.dr-root (the shadow-side equivalents).
const LIGHT_VARS = `
...
/* Obsidian's theme class wins over the OS scheme inside the modal shadow root. */
