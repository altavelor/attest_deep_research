...
- **Barrel для обратной совместимости.** При разбиении публичного модуля оставь исходный файл реэкспортом (`export * from "./..."`), чтобы внешние импорты не ломались (пример: `src/adapters/settings/settings.ts`).
