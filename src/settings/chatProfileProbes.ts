...
export function startChatProfileProbes<TTools, TResponses, TReasoning = never>(
  ...
  startProbe(tasks.probeTools, tasks.onTools, tasks.onError);
  startProbe(tasks.probeResponses, tasks.onResponses, tasks.onError);
  if (tasks.onReasoning) startProbe(tasks.probeReasoning, tasks.onReasoning, tasks.onError);
}
