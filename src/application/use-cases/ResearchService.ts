...
import { summarizeCompactionWithModel } from "./ChatCompaction";
...
import { ContextDiagnostics, ContextIndexDiagnostics, ContextMode, IndexDescriptionPromptContext, ResearchExecutionStrategy, ToolCapabilityProbeAudit, WebContextDiagnostics } from "../../core/diagnostics";
...
import { EvidencePlanner, EvidencePlannerOptions } from "../../core/research/EvidencePlanner";
import {
  NoteToolService,
...
import { extractFollowUpQuestions } from "../../core/research/prompts";
...
export class ResearchService implements ConversationEngine {
  ...
  private readonly toolsetFactory: ResearchToolsetFactory;
  private readonly modelRoundFactory: (chatModel: ChatModelProvider) => ModelRoundProvider;
  ...
  constructor(options: ResearchServiceOptions) {
    ...
    this.modelRoundFactory = options.modelRoundFactory;
  ...
  private async answerAgentically(options: {
    ...
    const created = this.toolsetFactory({
      availability: {
    ...
    if (this.contextLimitTokens && estimatedTokens > this.contextLimitTokens) {
      ...
      result = await new AgenticResearchRunner({
        modelRound: this.modelRound ?? this.modelRoundFactory(this.chatModel),
...
function isChunkList(value: unknown): value is {
  chunks: Array<{
    id: string;
    text: string;
    evidenceSource: import("../../core/model/source").SourceReference;
  }>;
} {
